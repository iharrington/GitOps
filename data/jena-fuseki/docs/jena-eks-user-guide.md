---
id: jena-eks-user-guide
title: Jena-Fuseki EKS User Guide
description: How to connect to the EKS cluster and work with Jena-Fuseki safely
---

# 🚀 Jena-Fuseki EKS User Guide

This guide explains how to connect to the EKS cluster, set up your tools, and interact with Jena-Fuseki pods safely.
...
To avoid typing `-n jena-fuseki` every time:

```bash
kubectl config set-context --current --namespace=jena-fuseki
```

Verify:
```bash
kubectl get pods -n jena-fuseki
```
...
### 🔌 Port-Forward Jena-Fuseki
This makes Jena-Fuseki available locally at `http://localhost:8080`.

```bash
kubectl port-forward svc/jena-fuseki-svc 8080:3030 -n jena-fuseki
```
...
```bash
kubectl logs -f -l app.kubernetes.io/name=jena-fuseki -n jena-fuseki
```
...
```bash
kubectl delete pod -l app.kubernetes.io/name=jena-fuseki -n jena-fuseki
```
...
- ✅ Always use the **`jena-fuseki` namespace**
